...
# 1. Security Group for the Public-Facing Lambda (Private Subnet with Egress)
...
# 2. Security Group for the Isolated Lambda (Isolated Subnet)
resource "aws_security_group" "isolated_lambda_sg" {
  ...
  # Ingress: Only allow HTTPS traffic coming from the Public Lambda Security Group (Chaining)
  ingress {
    from_port       = 443
    to_port         = 443
    protocol        = "tcp"
...
# 3. AWS WAFv2 Web ACL for the Public API Gateway
